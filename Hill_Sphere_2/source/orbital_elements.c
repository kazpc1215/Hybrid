...
int Calculate_OrbitalElements(int i,double x_c[][4],double v_c[][4],struct orbital_elements ele[],double P[][4],double Q[][4],double r_c[],double v2_c[],double r_dot_v[]){
  ...
  ele[i].axis = 1.0/(2.0/r_c[i] - v2_c[i]);
#else
...
#if !defined(G) && !defined(M_0)
  ele[i].ecc = sqrt((1.0-r_c[i]/ele[i].axis)*(1.0-r_c[i]/ele[i].axis) + r_dot_v[i]*r_dot_v[i]/ele[i].axis);
#else
...
#if !defined(G) && !defined(M_0)
    esin_u = r_dot_v[i]/sqrt(ele[i].axis);
#else
...
#if !defined(G) && !defined(M_0)
    P[i][k] = x_c[i][k]*cos(ele[i].u)/r_c[i] - sqrt(ele[i].axis)*v_c[i][k]*sin(ele[i].u);
    Q[i][k] = (x_c[i][k]*sin(ele[i].u)/r_c[i] + sqrt(ele[i].axis)*v_c[i][k]*(cos(ele[i].u)-ele[i].ecc))/sqrt(fabs(1.0-ele[i].ecc));
#else
...
#if !defined(G) && !defined(M_0)
    v_0[i][k] = sqrt(1.0/ele[i].axis)/r_0[i]*(-ele[i].axis*P[i][k]*sin(ele[i].u) + ele[i].axis*sqrt(1.0-ele[i].ecc*ele[i].ecc)*Q[i][k]*cos(ele[i].u));
#else
    v_0[i][k] = sqrt(1.0/ele[i].axis)/r_0[i]*(-ele[i].axis*P[i][k]*sin(ele[i].u) + ele[i].axis*sqrt(1.0-ele[i].ecc*ele[i].ecc)*Q[i][k]*cos(ele[i].u));
#endif
  }
