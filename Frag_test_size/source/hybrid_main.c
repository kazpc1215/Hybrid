...
#pragma omp parallel
  ...
  while(t_sys + t_tmp <= T_MAX){
	...
	////////////////////////////////////////////////////////////////
      ...
      if(t_[i_sys] + t_tmp + dt_[i_sys] > frag[i_sys].t_frag + frag[i_sys].dt_frag){
	...
	mass_tot_all += MassDepletion(i,ele[i].mass,(t_sys+t_tmp),frag);  //質量を予測してから足す.
      ...
      for(i=1;i<=tracerlistnumber;++i){
	//mass_tot_center += ele[i].mass;
	mass_tot_center += MassDepletion(tracerlist[i],ele[tracerlist[i]].mass,(t_sys+t_tmp),frag);  //質量を予測してから足す.
	n_center++;
      }
