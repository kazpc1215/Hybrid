...
int main(void){
  ...
  */
  ...
  for(i=1;i<=(int)(N_tr*4.0/5.0);++i){  //小さい方 1E24g
    ele[i].mass = M_TOT*5.0/8.0/(double)N_tr;  //トレーサー質量
  }

  for(i=(int)(N_tr*4.0/5.0)+1;i<=N_tr;++i){  //大きい方 4E24g
    ele[i].mass = M_TOT*5.0/2.0/(double)N_tr;  //トレーサー質量
  }
