...
#pragma omp parallel
  ...
  */
  ...
  if(stat(tempreadfile,&st)==0){  //読み込むファイルの存在確認.
    ...
    printf("initial mass depletion timescale =%e [yr] (center)\n",tau_dep_center/2.0/M_PI);
  ...
  fprintf(fptempfragread,"\n\n");
  fprintf(fptempfragread,"#i\tfragtimes\tt_frag[yr]\tdt_frag[yr]\n");

  for(i=global_n_p+1;i<=global_n;++i){
    fprintf(fptempfragread,"%d\t%d\t%.15e\t%.15e\n",
	    i,
	    frag[i].fragtimes,
	    frag[i].t_frag/2.0/M_PI,
	    frag[i].dt_frag/2.0/M_PI
