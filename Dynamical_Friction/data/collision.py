...
for subnum in range(1, SUBDIR_NUM+1):
    ...
    for n_col in range(1, N_tr+1):
        ...
        if os.path.exists(datafile):
            ...
            ##########
            Lx_p = mass_p * (y[subnum-1, 0] * vz[subnum-1, 0] - z[subnum-1, 0] * vy[subnum-1, 0]) - dL_x
            Ly_p = mass_p * (z[subnum-1, 0] * vx[subnum-1, 0] - x[subnum-1, 0] * vz[subnum-1, 0]) - dL_y
            Lz_p = mass_p * (x[subnum-1, 0] * vy[subnum-1, 0] - y[subnum-1, 0] * vx[subnum-1, 0]) - dL_z
            ...
            total_Lx = Lx_p + Lx_i.sum()
            total_Ly = Ly_p + Ly_i.sum()
            total_Lz = Lz_p + Lz_i.sum()
            ...
            dL_x += (mass_p + mass_tr) * (y_g12 * vz_g12 - z_g12 * vy_g12) - mass_p * (y[subnum-1, 0] * vz[subnum-1, 0] - z[subnum-1, 0] * vy[subnum-1, 0]) - mass_tr * (y[subnum-1, j_col-1] * vz[subnum-1, j_col-1] - z[subnum-1, j_col-1] * vy[subnum-1, j_col-1])
            dL_y += (mass_p + mass_tr) * (z_g12 * vx_g12 - x_g12 * vz_g12) - mass_p * (z[subnum-1, 0] * vx[subnum-1, 0] - x[subnum-1, 0] * vz[subnum-1, 0]) - mass_tr * (z[subnum-1, j_col-1] * vx[subnum-1, j_col-1] - x[subnum-1, j_col-1] * vz[subnum-1, j_col-1])
            dL_z += (mass_p + mass_tr) * (x_g12 * vy_g12 - y_g12 * vx_g12) - mass_p * (x[subnum-1, 0] * vy[subnum-1, 0] - y[subnum-1, 0] * vx[subnum-1, 0]) - mass_tr * (x[subnum-1, j_col-1] * vy[subnum-1, j_col-1] - y[subnum-1, j_col-1] * vx[subnum-1, j_col-1])
            print(n_col, dL_x, dL_y, dL_z)
            print("")
            ##########


            with open(directory + subdirectory + "Collision_time.dat", "a") as f:
                print(time[subnum-1, 0], "\t", N_tr - n_col, "\t", abs((total_energy - total_energy_0)/total_energy_0), "\t", dE_correct / total_energy_0, file=f, sep="")
