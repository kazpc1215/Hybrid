...
for subnum in range(1, SUBDIR_NUM+1):
    ...
    if(N_p == 1):
        ...
        ecc_p_rms[subnum-1, :] = arr[:, 2] ** 2
        ecc_tr_rms[subnum-1, :] = arr[:, 3] ** 2
        inc_1[subnum-1, :] = arr[:, 4] ** 2
        inc_p_rms[subnum-1, :] = arr[:, 5] ** 2
        inc_tr_rms[subnum-1, :] = arr[:, 6] ** 2
    elif(N_p == 3):
        time[subnum-1, :] = arr[:, 0]
        ecc_1[subnum-1, :] = arr[:, 1] ** 2
        ecc_2[subnum-1, :] = arr[:, 2] ** 2
        ecc_3[subnum-1, :] = arr[:, 3] ** 2
        ecc_p_rms[subnum-1, :] = arr[:, 4] ** 2
        ecc_tr_rms[subnum-1, :] = arr[:, 5] ** 2
        inc_1[subnum-1, :] = arr[:, 6] ** 2
        inc_2[subnum-1, :] = arr[:, 7] ** 2
        inc_3[subnum-1, :] = arr[:, 8] ** 2
        inc_p_rms[subnum-1, :] = arr[:, 9] ** 2
        inc_tr_rms[subnum-1, :] = arr[:, 10] ** 2

    print(subnum, time[subnum-1, 1], np.sqrt(ecc_p_rms[subnum-1, 1]), np.sqrt(inc_p_rms[subnum-1, 1]))
...
if(N_p == 1):
    ...
    rand_rms[3, :] = np.sqrt(ecc_p_rms.mean(axis=0))
    rand_rms[4, :] = 0.5 * ecc_p_rms.std(axis=0, ddof=1) / np.sqrt(ecc_p_rms.mean(axis=0))
    rand_rms[5, :] = np.sqrt(ecc_tr_rms.mean(axis=0))
    rand_rms[6, :] = 0.5 * ecc_tr_rms.std(axis=0, ddof=1) / np.sqrt(ecc_tr_rms.mean(axis=0))
    rand_rms[7, :] = np.sqrt(inc_1.mean(axis=0))
    rand_rms[8, :] = 0.5 * inc_1.std(axis=0, ddof=1) / np.sqrt(inc_1.mean(axis=0))
    rand_rms[9, :] = np.sqrt(inc_p_rms.mean(axis=0))
    rand_rms[10, :] = 0.5 * inc_p_rms.std(axis=0, ddof=1) / np.sqrt(inc_p_rms.mean(axis=0))
    rand_rms[11, :] = np.sqrt(inc_tr_rms.mean(axis=0))
    rand_rms[12, :] = 0.5 * inc_tr_rms.std(axis=0, ddof=1) / np.sqrt(inc_tr_rms.mean(axis=0))
elif(N_p == 3):
    ...
    rand_rms[7, :] = np.sqrt(ecc_p_rms.mean(axis=0))
    rand_rms[8, :] = 0.5 * ecc_p_rms.std(axis=0, ddof=1) / np.sqrt(ecc_p_rms.mean(axis=0))
    rand_rms[9, :] = np.sqrt(ecc_tr_rms.mean(axis=0))
    rand_rms[10, :] = 0.5 * ecc_tr_rms.std(axis=0, ddof=1) / np.sqrt(ecc_tr_rms.mean(axis=0))
    ...
    rand_rms[17, :] = np.sqrt(inc_p_rms.mean(axis=0))
    rand_rms[18, :] = 0.5 * inc_p_rms.std(axis=0, ddof=1) / np.sqrt(inc_p_rms.mean(axis=0))
    rand_rms[19, :] = np.sqrt(inc_tr_rms.mean(axis=0))
    rand_rms[20, :] = 0.5 * inc_tr_rms.std(axis=0, ddof=1) / np.sqrt(inc_tr_rms.mean(axis=0))
...
if(N_p == 1):
    ...
    plt.plot(time[1, :], rand_rms[3, :], color="r", label=r"$e_{\rm p}$")
    ...
    plt.plot(time[1, :], rand_rms[3, :], color="m", linestyle="dashed", linewidth=0.5, label=r"$e_{\rm p,2}$")
    plt.errorbar(time[1, :], rand_rms[3, :], yerr=rand_rms[4, :], fmt=".")
    plt.plot(time[1, :], rand_rms[5, :], color="y", linestyle="dashed", linewidth=0.5, label=r"$e_{\rm p,3}$")
    plt.errorbar(time[1, :], rand_rms[5, :], yerr=rand_rms[6, :], fmt=".")
    plt.plot(time[1, :], rand_rms[7, :], color="r", label=r"$e_{\rm p,rms}$")
    plt.errorbar(time[1, :], rand_rms[7, :], yerr=rand_rms[8, :], fmt=".")
    plt.plot(time[1, :], rand_rms[9, :], color="b", label=r"$e_{\rm tr,rms}$")
    plt.errorbar(time[1, :], rand_rms[9, :], yerr=rand_rms[10, :], fmt=".")
...
if(N_p == 1):
    plt.title(r"$N_{\rm tr}=1000,M_{\rm tot}=10 {\rm M_{\oplus}}$", fontsize=18)
    plt.plot(time[1, :], rand_rms[9, :], color="r", label=r"$i_{\rm p}$")
    plt.errorbar(time[1, :], rand_rms[9, :], yerr=rand_rms[10, :], fmt=".")
    plt.plot(time[1, :], rand_rms[11, :], color="b", label=r"$i_{\rm tr,rms}$")
    plt.errorbar(time[1, :], rand_rms[11, :], yerr=rand_rms[12, :], fmt=".")
elif(N_p == 3):
    plt.title(r"$N_{\rm tr}=3000,M_{\rm tot}=30 {\rm M_{\oplus}}$", fontsize=18)
    plt.plot(time[1, :], rand_rms[11, :], color="c", linestyle="dashed", linewidth=0.5, label=r"$i_{\rm p,1}$")
    plt.errorbar(time[1, :], rand_rms[11, :], yerr=rand_rms[12, :], fmt=".")
    plt.plot(time[1, :], rand_rms[13, :], color="m", linestyle="dashed", linewidth=0.5, label=r"$i_{\rm p,2}$")
    plt.errorbar(time[1, :], rand_rms[13, :], yerr=rand_rms[14, :], fmt=".")
    plt.plot(time[1, :], rand_rms[15, :], color="y", linestyle="dashed", linewidth=0.5, label=r"$i_{\rm p,3}$")
    plt.errorbar(time[1, :], rand_rms[15, :], yerr=rand_rms[16, :], fmt=".")
    plt.plot(time[1, :], rand_rms[17, :], color="r", label=r"$i_{\rm p,rms}$")
    plt.errorbar(time[1, :], rand_rms[17, :], yerr=rand_rms[18, :], fmt=".")
    plt.plot(time[1, :], rand_rms[19, :], color="b", label=r"$i_{\rm tr,rms}$")
    plt.errorbar(time[1, :], rand_rms[19, :], yerr=rand_rms[20, :], fmt=".")
