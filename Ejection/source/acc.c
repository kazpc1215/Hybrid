...
double Acceleration_ij(int i,int j,int k,struct orbital_elements ele[],double x_0[][4],double abs_r2[]){
  ...
  //return G*ele[j].mass*(x_0[j][k] - x_0[i][k])/(abs_r2[j] + EPSILON*EPSILON)/sqrt(abs_r2[j] + EPSILON*EPSILON);
}
...
double dAcceleration_ij(int i,int j,int k,struct orbital_elements ele[],double x_0[][4],double v_0[][4],double r_dot_v_ij[],double abs_r2[]){
  ...
  //return G*ele[j].mass*((v_0[j][k] - v_0[i][k])/(abs_r2[j] + EPSILON*EPSILON)/sqrt(abs_r2[j] + EPSILON*EPSILON) - 3*r_dot_v_ij[j]*(x_0[j][k] - x_0[i][k])/(abs_r2[j] + EPSILON*EPSILON)/(abs_r2[j] + EPSILON*EPSILON)/sqrt(abs_r2[j] + EPSILON*EPSILON));
}
