...
int main(void){
  ...
  */
  ...
  for(i=global_n_p+1;i<=global_n;++i){  //微惑星.
    ...
    sprintf(ele[i].name,"tracer%04d",i-global_n_p);
    ele[i].mass = PLANET_MASS/10.0/(double)(global_n-global_n_p);  //微惑星の総質量は原始惑星の1/10.
    ele[i].axis = ((double)rand())/((double)RAND_MAX+1.0)*(OUTER_AXIS-INNER_AXIS)+INNER_AXIS;  //
