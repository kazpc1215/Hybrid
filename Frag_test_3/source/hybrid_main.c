...
int main(void){
  ...
  para.Q_D = Q_0_FRAG*1.13E-13*pow(RHO/3.0,0.55)*pow(M_MAX*2.0E12,P_FRAG);  //erg/g = 1.13E-13 AU^2/(yr/2pi)^2 として
  ...
  for(i=1;i<=N_tr;++i){
    ...
    */
    ...
    printf("i=%d\tv_ave=%f\tsigma=%f\tecc=%f\tr_0=%f\tdelta_r=%f\n",i,frag[i].v_ave,frag[i].sigma,ele[i].ecc,r_0[i],frag[i].delta_r);
  ...
  for(i=1;i<=N_tr;i++){
    fprintf(fpposi,"%e\t%04d\t%.15f\t%.15f\t%.15f\t%.15f\t%.15f\t%.15f\t%.15f\t%.15f\t%.15f\t%d\n",0.0,i,x_0[i][1],x_0[i][2],x_0[i][3],r_0[i],sqrt(x_0[i][1]*x_0[i][1]+x_0[i][2]*x_0[i][2]),ele[i].mass,frag[i].delta_r,frag[i].sigma,frag[i].n_s,frag[i].neighbornumber);
  }
  fprintf(fpposi,"\n\n");
  ...
  while(TIME_INTERVAL_MAX>interval){
	...
	fprintf(fporbit,"%e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\n",t_sys,ele[i].ecc,ele[i].axis,ele[i].u,ele[i].inc,ele[i].Omega,ele[i].omega,ele[i].r_h);
	...
	*/
	...
	return -1;
      ...
      for(i=1;i<=N_tr;i++){
	fprintf(fpposi,"%e\t%04d\t%.15f\t%.15f\t%.15f\t%.15f\t%.15f\t%.15f\t%.15f\t%.15f\t%.15f\t%d\n",t_sys/2.0/M_PI,i,x_c[i][1],x_c[i][2],x_c[i][3],r_c[i],sqrt(x_c[i][1]*x_c[i][1]+x_c[i][2]*x_c[i][2]),ele[i].mass,frag[i].delta_r,frag[i].sigma,frag[i].n_s,frag[i].neighbornumber);
      }
      fprintf(fpposi,"\n\n");
      fclose(fpposi);
