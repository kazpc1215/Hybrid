...
#define DIRECTORY ../data/Ejection_L2cone30equidistant_v1011curl_Frag_OnlyPlanet_xi001_n1000_10yr/  //ファイル保存用のディレクトリ.
...
#define PLANET_INC 0.001
#define PLANET_U 0.0
#define PLANET_omega 0.0
#define PLANET_Omega 0.0
...
//////////////////////////////////////////////////
#define T_MAX (2.0*M_PI*1.0E1)  //1E1yr  全計算時間.

#define DT_LOG 1  // 1: t_eneをlogでとる 0: t_eneをlinearでとる

/*  log 用 */
#define TIME_INTERVAL 2.0*M_PI*1.0E-6,2.0*M_PI*1.0E-5,2.0*M_PI*1.0E-4,2.0*M_PI*1.0E-3,2.0*M_PI*1.0E-2,2.0*M_PI*1.0E-1,2.0*M_PI*1.0E0,T_MAX  //t_ene配列の中身.
#define TIME_INTERVAL_MAX 8 //t_ene配列の要素数.
...
struct execution_time{
  ...
  double SquareOfVelocity;
  double Timestep_i_0;
  double Timestep_i_sys;
  double SquareOfRelativeDistance;
  double SquareOfRelativeVelocity;
  double RelativeInnerProduct;
  double Acceleration_ij;
  double dAcceleration_ij;
  double Acceleration_indirect;
  double dAcceleration_indirect;
  double External_Acceleration;
  double External_dAcceleration;
  double All_Acceleration;
  double All_dAcceleration;
  double Calculate_OrbitalElements;
  double Calculate_P;
  double Calculate_Q;
  double Calculate_Energy;
  double InitialCondition;
  double Predictor;
  double Corrector_sys;
  double Iteration_sys;
  ...
  double MassDepletion;
  double s_1_FRAG_trapezoid;
  double s_2_FRAG_trapezoid;
  double s_3_FRAG_trapezoid;
  double s_1_FRAG_integrand;
  double s_2_FRAG_integrand;
  double s_3_FRAG_integrand;
  double s_1_FRAG;
  double s_2_FRAG;
  double s_3_FRAG;
  double Rotation_3D_xaxis;
  double Rotation_3D_yaxis;
  double Rotation_3D_zaxis;
  double Swap;
